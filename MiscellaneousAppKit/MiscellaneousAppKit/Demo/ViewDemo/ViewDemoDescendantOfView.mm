...
@interface ViewDemoDescendantOfView ()
...
@implementation ViewDemoDescendantOfView
...
- (instancetype)initWithFrame:(NSRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        self.secondaryView.stringValue = [NSString stringWithFormat:@"Secondary View - Is Descendant Of Primary View : %@", [self.secondaryView isDescendantOf:self.primaryView] ? @"YES" : @"NO"];
        self.tertiaryView.stringValue = [NSString stringWithFormat:@"Tertiary View - Is Descendant Of Primary View : %@", [self.tertiaryView isDescendantOf:self.primaryView] ? @"YES" : @"NO"];
    }
...
- (void)layout {
    ...
    self.tertiaryView.frame = NSInsetRect(self.bounds, 40., 40.);
}
